...
func (l ColorLogger) output(_, f string, args ...interface{}) {
...
func NewTCPProxy(lconn net.Conn, dialer func() (net.Conn, error), inmem *InmemSocket) *Proxy {
	return &Proxy{
		lconn:  lconn,
		dialer: dialer,
		erred:  false,
...
func (p *Proxy) err(s string, err error) {
	p.Log.Warn("%v Err (%v) %v", time.Now().Format(time.TimeOnly), s, err)
	if p.erred {
		return
	}
...
func (p *Proxy) pipe(src, dst io.ReadWriter) {
	...
	for {
		...
		//show output
		p.Log.Debug("%v: "+dataDirection, time.Now().Format(time.TimeOnly), n, "")
